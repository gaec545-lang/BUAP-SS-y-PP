...
def register_student(req: dict, request: Request, db: Session = Depends(get_db)):
    ...
    # Task 3: Prevent registration with different matricula but same name
    if db.query(models.OpsStudent).filter_by(full_name=full_name).first():
        raise HTTPException(status_code=409, detail="Ya existe un alumno registrado con este mismo nombre y apellidos. Si tienes un problema con tu cuenta, contacta a la coordinación.")
