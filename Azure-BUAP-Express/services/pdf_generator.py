...
def generate_pdf_for_student(
    ...
    **extra_data
    ...
    # 1. Extraemos info del programa desde el enrollment de manera global para habilitar fallbacks consistentes
    program = None
    ...
    # 2. Resolución robusta del responsable y su cargo con fallbacks encadenados
    final_responsible = (
        addressed_to or 
        responsible_name or 
        (program.responsible_name if program and program.responsible_name else None) or 
        (program.evaluator_name if program and program.evaluator_name else None) or 
        evaluator_name or 
        "A QUIEN CORRESPONDA"
    )
    final_position = (
        responsible_position or 
        (program.responsible_position if program and program.responsible_position else "") or 
        ""
    )
        ...
        if document_type in template_map:
            ...
            if os.path.exists(template_path):
                ...
                if document_type == 'DOC-SS-002':
                    data_dict = {
                        'nombre nombramiento': final_responsible,
                        ...
                        'nombre responsable': final_responsible,
                        'cargo responsable': final_position,
                        'responsable': final_responsible,
                        'cargo': final_position,
                    }
                    ...
                    data_dict = {
                        ...
                        'responsable': final_responsible,
                        'cargo': final_position,
                        'carta asignacion dirigida a': final_responsible,
                    }
    ...
    _add_document_body(story, document_type, student, body_style, section_style,
                       dependency_name=dependency_name, career_name=career_name, addressed_to=final_responsible)
    ...
    sig_data = [
        ...
        ['', '', ''],
        ['_' * 30, '', '_' * 30],
        [student.full_name, '', final_responsible],
        ['(Firma en tinta azul)', '', final_position if final_position else '(Firma en sello y tinta azul)'],
    ]
