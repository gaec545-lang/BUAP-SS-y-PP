...
class DimProgram(Base):
    ...
    responsible_position = Column(String(200))
    ...
    __table_args__ = (UniqueConstraint('folio', 'career_id', name='uq_folio_career'),)
...
class OpsExcelJob(Base):
    __tablename__ = "ops_excel_jobs"
    id = Column(String(50), primary_key=True, index=True)
    status = Column(String(20), default="pending")  # "pending", "running", "done", "error"
    result = Column(Text)                           # JSON storage
    created_at = Column(DateTime, default=func.now())
    updated_at = Column(DateTime, default=func.now(), onupdate=func.now())
...
class OpsValidatorUser(Base):
    __tablename__ = "ops_validator_users"
    id = Column(Integer, primary_key=True, autoincrement=True)
    username = Column(String(50), unique=True, nullable=False)
    password_hash = Column(String(200), nullable=False)
    full_name = Column(String(200), nullable=False)
    role = Column(String(20), default="viewer")  # coordinador | viewer
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, default=func.now())


class FactValidationRun(Base):
    __tablename__ = "fact_validation_runs"
    id = Column(Integer, primary_key=True, autoincrement=True)
    ...
    validated_by_id = Column(Integer, ForeignKey("ops_validator_users.id"), nullable=False)
    validated_at = Column(DateTime, default=func.now())
    overall_result = Column(String(20), nullable=False)  # pass|fail|warning|manual_review
    override_result = Column(String(20), nullable=True)
    override_reason = Column(Text, nullable=True)
    override_by = Column(Integer, nullable=True)
    override_at = Column(DateTime, nullable=True)
    notes = Column(Text, nullable=True)
    raw_extracted_text = Column(Text, nullable=True)
    extraction_method = Column(String(20), nullable=True)  # text_layer|ocr|hybrid
    overall_confidence = Column(Float, nullable=True)
    read_status = Column(String(50), nullable=True)

    checks = relationship("FactValidationCheck", back_populates="run", cascade="all, delete-orphan")
    validator = relationship("OpsValidatorUser")


class FactValidationCheck(Base):
    __tablename__ = "fact_validation_checks"
    id = Column(Integer, primary_key=True, autoincrement=True)
    run_id = Column(Integer, ForeignKey("fact_validation_runs.id"), nullable=False)
    check_name = Column(String(100), nullable=False)
    result = Column(String(20), nullable=False)  # pass|fail|warning|skipped
    extracted_value = Column(String(500), nullable=True)
    expected_value = Column(String(500), nullable=True)
    confidence = Column(Float, nullable=True)
    detail = Column(Text, nullable=True)

    run = relationship("FactValidationRun", back_populates="checks")


VALIDATOR_TABLES = [
    OpsValidatorUser.__table__,
    FactValidationRun.__table__,
    FactValidationCheck.__table__,
]
